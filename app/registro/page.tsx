...
function VinylDeco({
  size, accent, speed, reverse = false, style,
}: {
  size: number; accent: string; speed: string; reverse?: boolean; style?: React.CSSProperties;
}) {
  return (
    <div style={{
      ...
      background: `radial-gradient(circle,
        ${accent} 0%, ${accent} 17%,
        #141414 18%, #141414 24%, #222 25%, #141414 27%,
        #222 30%, #141414 32%, #222 35%, #141414 37%,
        #222 40%, #141414 42%, #222 46%, #141414 50%,
        #222 55%, #141414 60%, #222 66%, #141414 72%,
        #222 80%, #141414 86%, #1e1e1e 100%
      )`,
...
function getPasswordStrength(pw: string): { score: number; label: string; color: string } {
  ...
  const levels = [
    { score: 1, label: "Débil",    color: "#ff4d4d" },
    { score: 2, label: "Regular",  color: "#ffaa00" },
    { score: 3, label: "Buena",    color: "#5eead4" },
    { score: 4, label: "Fuerte",   color: "#1CF094" },
  ];
...
export default function RegisterPage() {
  ...
  const canSubmit = name && email && password && confirm && !mismatch && strength.score >= 2 && !loading;
  ...
  return (
    <div style={{
      ...
      <div className="ring-deco" style={{ width: "560px", height: "560px", animationDelay: "1.5s", borderColor: "rgba(28,240,148,0.05)" }} />

      {/* Vinilos flotantes */}
      <div style={{ position: "absolute", left: "4%", top: "12%", opacity: 0.55, animation: "float-y-slow 6s ease-in-out infinite", pointerEvents: "none" }}>
        <VinylDeco size={170} accent="#a3ff47" speed="9s" />
      </div>
      <div style={{ position: "absolute", right: "5%", bottom: "14%", opacity: 0.45, animation: "float-y-slow 7s ease-in-out infinite 1.5s", pointerEvents: "none" }}>
        <VinylDeco size={130} accent="#1CF094" speed="7s" reverse />
      </div>
      <div style={{ position: "absolute", right: "10%", top: "8%", opacity: 0.28, animation: "float-y 8s ease-in-out infinite 0.6s", pointerEvents: "none" }}>
        <VinylDeco size={95} accent="#5eead4" speed="11s" />
      </div>
      <div style={{ position: "absolute", left: "8%", bottom: "8%", opacity: 0.3, animation: "float-y 9s ease-in-out infinite 2s", pointerEvents: "none" }}>
      ...
      {MUSIC_NOTES.map((note, i) => (
        <div key={i} style={{
          position: "absolute", left: `${15 + i * 14}%`, bottom: "15%",
          color: "#1CF094", fontSize: `${1 + (i % 3) * 0.4}rem`,
      ...
      <div style={{
        ...
        height: "70px", width: "340px", opacity: 0.14, pointerEvents: "none",
      }}>
        {EQ_BARS.map((bar, bi) => (
          <div key={bi} style={{
            flex: 1, height: `${bar.height}px`,
      ...
      <div style={{
        ...
        background: "radial-gradient(ellipse at center, rgba(28,240,148,0.055) 0%, transparent 68%)",
      ...
      <div style={{
        ...
        boxShadow: "0 30px 90px rgba(0,0,0,0.7), 0 0 0 1px rgba(28,240,148,0.07), inset 0 1px 0 rgba(255,255,255,0.06)",
        animation: mounted ? "card-in 0.75s cubic-bezier(0.34,1.56,0.64,1) both" : "none",
      }}>
        ...
        <div style={{
          ...
          height: "2px",
          background: "linear-gradient(90deg, transparent, #a3ff47, #1CF094, transparent)",
          ...
          <Link href="/">
            <img
              ...
              style={{
                ...
                animation: "logo-in 0.8s cubic-bezier(0.34,1.56,0.64,1) both, glow-pulse 3.5s ease-in-out 0.8s infinite",
                cursor: "pointer",
              }}
            />
          </Link>
          <div style={{ textAlign: "center", animation: mounted ? "fade-up 0.6s ease 0.25s both" : "none" }}>
            <h1 style={{
            ...
            <p style={{
              ...
              margin: "8px 0 0", fontFamily: "Arial, sans-serif", letterSpacing: "0.4px",
            }}>
              Únete a SpotiFake hoy
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{
                ...
                <div style={{ flex: 1, display: "flex", gap: "4px" }}>
                  {[1, 2, 3, 4].map((lvl) => (
                    <div key={lvl} style={{
                      ...
                      background: strength.score >= lvl ? strength.color : "rgba(255,255,255,0.1)",
                      transition: "background 0.3s ease",
                    }} />
                  ))}
                </div>
                <span style={{ color: strength.color, fontSize: "0.68rem", fontFamily: "Arial, sans-serif", minWidth: "44px" }}>
                  {strength.label}
                </span>
              </div>
            )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
            ...
            <input
              type="password"
              className={`reg-input${mismatch ? " error" : ""}`}
              placeholder="••••••••"
              autoComplete="new-password"
              value={confirm}
        ...
        <p style={{
          ...
          fontSize: "0.82rem", marginTop: "24px", marginBottom: 0,
          fontFamily: "Arial, sans-serif",
          animation: mounted ? "fade-up 0.6s ease 0.75s both" : "none",
        }}>
          ¿Ya tienes cuenta?{" "}
          <Link href="/usuario" style={{ color: "#1CF094", fontWeight: 700, textDecoration: "none" }}>
            Inicia sesión
          </Link>
        </p>
