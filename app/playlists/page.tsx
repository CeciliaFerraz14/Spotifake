...
type Playlist = { id: string; nombre: string; accent: string; bg: string; image_url?: string | null; canciones?: number };
...
const css = `
  ...
  .pl-card:hover .dots-btn { opacity: 1; }
  .dots-btn:hover { background: rgba(0,0,0,0.85); transform: scale(1.12); color: #1CF094; }
  .dots-btn.always-visible { opacity: 1; }
...
export default function PlaylistsPage() {
  ...
  const [coverTarget, setCoverTarget]           = useState<Playlist | null>(null);
  ...
  const eliminarPlaylist = async (pl: Playlist) => {
    setCtxMenu(null);
    if (!confirm(`�Eliminar la playlist �${pl.nombre}�? Esta acci�n no se puede deshacer.`)) return;
  ...
  const abrirPortada = (pl: Playlist) => {
    setCtxMenu(null);
    setCoverTarget(pl);
  ...
  const guardarPortada = async () => {
    ...
    try {
      if (coverFile) {
        ...
        const res = await fetch(`/api/playlists/${coverTarget.id}/portada`, { method: 'POST', body: fd });
        if (!res.ok) { setCoverError("No se pudo subir la imagen"); return; }
        const { image_url } = await res.json();
        setPlaylists(prev => prev.map(p => p.id === coverTarget.id ? { ...p, image_url } : p));
        ...
        if (coverTarget.image_url) body.image_url = null;
        const res = await fetch(`/api/playlists/${coverTarget.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        });
        if (!res.ok) { setCoverError("No se pudo cambiar el color"); return; }
        setPlaylists(prev => prev.map(p => p.id === coverTarget.id ? { ...p, accent: c.accent, bg: c.bg, image_url: null } : p));
      } else if (coverRemoveImage && coverTarget.image_url) {
        const res = await fetch(`/api/playlists/${coverTarget.id}/portada`, { method: 'DELETE' });
        if (!res.ok) { setCoverError("No se pudo quitar la imagen"); return; }
        setPlaylists(prev => prev.map(p => p.id === coverTarget.id ? { ...p, image_url: null } : p));
      }
      cerrarPortada();
    } finally {
  ...
  const abrirMenuDots = (pl: Playlist, e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
  ...
  return (
    <div style={{
                ...
                >
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      <div style={{
                        position: "absolute", inset: 0,
                        background: pl.image_url
                          ? "linear-gradient(180deg, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0.35) 100%)"
                          : `radial-gradient(circle at 30% 30%, ${pl.accent}22, transparent 70%)`,
                        pointerEvents: "none",
                      }} />
      ...
      {ctxMenu && (
        <>
          ...
          <div className="ctx-menu" style={{ top: ctxMenu.y, left: ctxMenu.x }}>
            ...
            <button className="ctx-item" onClick={() => abrirPortada(ctxMenu.playlist)}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
      ...
      {/* Modal: Cambiar portada */}
      {coverOpen && coverTarget && (
        <div className="rename-backdrop" onClick={cerrarPortada}>
          <div className="rename-box" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
              <h2 style={{ margin: 0, fontFamily: "var(--font-nunito), sans-serif", fontWeight: 900, fontSize: "1.1rem", color: "white" }}>
                Cambiar portada
              </h2>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginBottom: "20px" }}>
              <div style={{
                width: 120, height: 120, borderRadius: "14px", overflow: "hidden",
                background: coverColorIdx !== null ? COLOR_OPTIONS[coverColorIdx].bg : coverTarget.bg,
                display: "flex", alignItems: "center", justifyContent: "center",
                ...
                ) : !coverRemoveImage && coverTarget.image_url && coverColorIdx === null ? (
                  <img src={coverTarget.image_url} alt="actual" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                ) : (
                  <Vinyl accent={coverColorIdx !== null ? COLOR_OPTIONS[coverColorIdx].accent : coverTarget.accent} size={70} />
                )}
              </div>
            </div>
            ...
            <label className="upload-zone">
              ...
              ) : coverTarget.image_url && !coverRemoveImage ? (
                <img src={coverTarget.image_url} alt="actual" className="upload-thumb" />
              ) : (
            ...
            {coverTarget.image_url && !coverFile && !coverRemoveImage && (
